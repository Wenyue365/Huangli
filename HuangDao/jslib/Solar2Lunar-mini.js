...
function GetcDateString(){var tmp="";tmp+=tgString.charAt((cYear-4)%10);tmp+=dzString.charAt((cYear-4)%12);tmp+="(";tmp+=sx.charAt((cYear-4)%12);tmp+=")年 ";if(cMonth<1){tmp+="(闰)";tmp+=monString.charAt(-cMonth-1);}else{tmp+=monString.charAt(cMonth-1);}
tmp+="月";tmp+=(cDay<11)?"初":((cDay<20)?"十":((cDay<30)?"廿":"三十"));if(cDay%10!=0||cDay==10){tmp+=numString.charAt((cDay-1)%10);}
return tmp;}
function GetLunarInfo(year,month,day){if(year<1921||year>2020){return"";}else{month=(parseInt(month)>0)?(month-1):11;e2c(year,month,day);}
var LunarInfo={year:"",month:"",day:"",zodiacYear:"",leapMonth:""}
var tmp="";tmp+=tgString.charAt((cYear-4)%10);tmp+=dzString.charAt((cYear-4)%12);LunarInfo.year=tmp;LunarInfo.zodiacYear=sx.charAt((cYear-4)%12);if(cMonth<1){LunarInfo.leapMonth="(闰)";LunarInfo.month=monString.charAt(-cMonth-1);}else{LunarInfo.month=monString.charAt(cMonth-1);}
LunarInfo.day=(cDay<11)?"初":((cDay<20)?"十":((cDay<30)?"廿":"三十"));if(cDay%10!=0||cDay==10){LunarInfo.day+=numString.charAt((cDay-1)%10);}
return LunarInfo;}
...
function GetCNDate(){var D=new Date();var yy=D.getFullYear();var mm=D.getMonth()+1;var dd=D.getDate();var ww=D.getDay();var ss=parseInt(D.getTime()/1000);if(yy<100)
yy="19"+yy;return GetLunarDay(yy,mm,dd);}